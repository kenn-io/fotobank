...
- Full Makefile, golangci-lint config, nilaway wiring, prek hooks,
  and mise toolchain pinning matching reference-project's conventions.
...
Copied from reference-project unless noted.
...
Example Makefile targets (copied/adapted from reference-project):
...
`.golangci.yml` mirrors reference-project's:
...
Rules copied verbatim from reference-project's CLAUDE.md:
...
- When a test function has more than three assertions, create a
  ...
  `testify-helper-check` tool (§3.6) ported from reference-project.
...
`prek.toml` mirrors reference-project's minus the frontend-check hook (no
frontend in Phase 1):
...
binary we port from reference-project: it refuses to edit migrations once they
...
starts the server. Matches reference-project's shape.

### 3.7 `testify-helper-check` tool

Tiny Go tool ported from reference-project. Rule: any `*_test.go` function
...
Mirrors reference-project's `config read`:
...
`tools/migrationhistorycheck` binary ported from reference-project runs as a
...
Two `*sql.DB` handles to the same file, mirroring reference-project's
`internal/db/db.go`:
...
`internal/cli/dispatch.go` follows reference-project's `runCLI` pattern:
