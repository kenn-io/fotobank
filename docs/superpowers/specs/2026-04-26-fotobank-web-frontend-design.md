...
>
> **Plan reference:** `~/code/reference-project` for SPA build/dev/embed patterns; `~/code/agentsview` for layout and theming inspiration.
...
### 13.1 Build / dev (reference-project pattern)
...
- `frontend/vite.config.ts` mirrors reference-project: dev proxy to backend, `host=127.0.0.1`, port `5181` (distinct from reference-project's 5174 so both can run together).
- No `packages/ui` workspace in v1.

### 13.2 Frontend dependencies (v1)

- `svelte`, `vite`, `typescript`, `@sveltejs/vite-plugin-svelte` — pin exact versions in `frontend/package.json` at scaffold time, mirroring reference-project's current versions. Bump only when Svelte tooling requires newer.
...
- `eslint`, `eslint-plugin-svelte`, `prettier` — for ecosystem fit with `svelte-check`; matches reference-project.
...
- CSS-variable-based theme (agentsview / reference-project pattern). Variables in `frontend/src/app.css`: `--bg-primary, --bg-surface, --bg-elevated, --text-primary, --text-secondary, --text-muted, --accent, --border, --shadow, --radius`.
