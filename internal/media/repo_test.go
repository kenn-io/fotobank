...
// idsOf collects the IDs of media rows in input order. Used by the
// SF-2 facet filter tests so assertions can sort and compare against
// expected ID sets.
...
// TestRepoList_AnyTagKeys filters on at-least-one tag (OR-composed
// via a single EXISTS).
