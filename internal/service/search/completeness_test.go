...
// TestCompleteness_HideMappedRowTracksAssertiveDrop pins the
// owner-scoped numerator: hiding a previously-mapped row drops both
// numerator and denominator by one in the visible-only count. The
// JOIN through media is what makes this honest — a cached read of
// embedding_generations.embedded_count would leave the numerator at 6
// while the denominator dropped to 6, clamping to 100% when the real
// visible-only ratio is 5/6.
//
// Concretely: 7 visible + 3 hidden, 6 visible mapped → 6/7 visible
// ratio. Hide the first mapped row → 5 visible mapped + 6 visible
// eligible → 5/6 (≈0.833).
func TestCompleteness_HideMappedRowTracksAssertiveDrop(t *testing.T) {
	r := require.New(t)
	d := testutil.OpenTestDB(t)
	...
	got, err := svc.EmbeddingCompleteness(context.Background(), owner, false, nil)
	r.NoError(err)
	r.InDelta(5.0/6.0, got, 0.001,
		"after hiding mids[0]: visible-mapped 5 / visible-eligible 6 = 0.833")
}
